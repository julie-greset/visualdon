...
var yScale = d3.scaleLinear().domain([0, d3.max(_data.default, function (d) {
  return d.nb;
})]).range([GRAPH_HEIGHT - MARGIN_TOP, 0]); // Echelle couleur

var fillScale = d3.scaleLinear().domain([0, d3.max(_data.default, function (d) {
  return d.nb;
})]) // données en entrée
...
.attr('fill', function (d) {
  return fillScale(d.nb);
}); //.attr('fill', 'steelblue')

var genreNames = svg.append('g').attr('transform', "translate(".concat(MARGIN_LEFT, ", 0)"));
genreNames.selectAll('text').data(_data.default).enter().append('text').attr('x', function (d, i) {
  return i * BAR_WIDTH + BAR_WIDTH / 2;
}).attr('y', HEIGHT - MARGIN_BOTTOM + 20).attr('text-anchor', 'middle').attr('font-family', 'sans-serif').attr('font-size', 13).text(function (d) {
  return d.genre;
});
var axisY = d3.axisLeft().scale(yScale).tickFormat(function (d) {
  return "".concat(d / 1, " titres");
}).ticks(5);
